...
fn strict_word_prefixes(element: &StrictXmlElement) -> Vec<String> {
    let mut prefixes = Vec::new();
    for (prefix, namespace) in element.raw_xml().namespaces().bindings() {
        prefixes.push(namespace_binding(prefix, namespace));
        if namespace == W_NS {
...
fn strict_extra_attributes(
    element: &StrictXmlElement,
    modelled: &[&[u8]],
) -> Result<Vec<(String, String)>> {
    let inherited = strict_word_prefixes(element);
    let mut reader = Reader::from_reader(element.raw_xml().bytes());
    let mut buffer = Vec::new();
    match reader.read_event_into(&mut buffer)? {
        Event::Start(start) | Event::Empty(start) => {
            let prefixes = word_prefixes_at(&start, &inherited)?;
            capture_extra_attributes(&start, modelled, &prefixes)
        }
        _ => Err(OxmlError::UnexpectedElement(
            "expected strict XML element start".to_string(),
        )),
    }
}

fn strict_root_attributes(element: &StrictXmlElement) -> Result<Vec<(String, String)>> {
    let mut reader = Reader::from_reader(element.raw_xml().bytes());
    let mut buffer = Vec::new();
    match reader.read_event_into(&mut buffer)? {
        Event::Start(start) | Event::Empty(start) => capture_root_attributes(&start),
        _ => Err(OxmlError::UnexpectedElement(
            "expected numbering root element start".to_string(),
        )),
    }
}
...
impl CT_Lvl {
    ...
    fn from_strict_xml(element: StrictXmlElement) -> Result<Self> {
        ...
        let parsed = element.parse(|cursor| {
            ...
            for index in 0..cursor.child_slots() {
                ...
                let child = take_element(cursor, index, local.unwrap_or("numbering level child"))?;
                let Some(local) = local else {
                    let prefixes = strict_word_prefixes(&child);
                    let (at, next) = level_raw_boundary(
                        child.raw_xml().name().qualified.as_bytes(),
                        boundary,
                        &prefixes,
                    );
                ...
                let completeness = match local {
                    ...
                    "rPr" => {
                        ...
                        let completeness = properties.completeness.clone();
                        if !completeness.is_complete() {
                            level.rpr_raw = Some((properties.clone(), raw, prefixes));
                        }
    ...
    fn to_xml_with_prefix<W: std::io::Write>(
        ...
        if let Some(ref style_id) = self.p_style {
            ...
            let val_name = qualified(word_prefix, "val");
            let mut e = BytesStart::new(name.as_str());
            e.push_attribute((val_name.as_str(), style_id.as_str()));
            writer.write_event(Event::Empty(e))?;
        }
        for boundary in 4..=6 {
            write_extras_at(writer, &self.extra_xml, boundary)?;
...
impl CT_AbstractNum {
    ...
    fn from_strict_xml(element: StrictXmlElement) -> Result<Self> {
        let extra_attributes = strict_extra_attributes(&element, &[b"abstractNumId"])?;
        let mut descendants = Vec::new();
        let parsed = element.parse(|cursor| {
            ...
            abstract_num.extra_attributes = extra_attributes;
            ...
            for index in 0..cursor.child_slots() {
                ...
                match kind {
                    Some("lvl") => {
                        ...
                        boundary = 7 + abstract_num.levels.len();
                    }
                    Some(local) => {
                        let parsed_value =
                    ...
                    None => {
                        let prefixes = strict_word_prefixes(&child);
                        let (at, next) = abstract_raw_boundary(
                            child.raw_xml().name().qualified.as_bytes(),
                            boundary,
                            &prefixes,
                        );
    ...
    fn to_xml_with_prefix<W: std::io::Write>(
        ...
        if let Some(ref style_id) = self.style_link {
            let name = qualified(word_prefix, "styleLink");
            let val_name = qualified(word_prefix, "val");
            let mut e = BytesStart::new(name.as_str());
            e.push_attribute((val_name.as_str(), style_id.as_str()));
            writer.write_event(Event::Empty(e))?;
        ...
        if let Some(ref style_id) = self.num_style_link {
            let name = qualified(word_prefix, "numStyleLink");
            let val_name = qualified(word_prefix, "val");
            let mut e = BytesStart::new(name.as_str());
            e.push_attribute((val_name.as_str(), style_id.as_str()));
            writer.write_event(Event::Empty(e))?;
...
pub struct CT_LvlOverride {
    ...
    #[doc(hidden)]
    pub source_original: Option<Box<CT_LvlOverride>>,
}
...
impl CT_LvlOverride {
        ...
        let parsed = element.parse(|cursor| {
            ...
            for index in 0..cursor.child_slots() {
                ...
                if child.is_named(Some(W_NS), "startOverride") && value.start_override.is_none() {
                    ...
                    let child = take_element(cursor, index, "level override child")?;
                    descendants.push(unmodeled_element_completeness(child.clone()));
                    value
                        .extra_xml
                        .push((position, child.into_raw_xml().bytes().to_vec()));
                }
            }
            Ok(value)
        })?;
    ...
    fn to_xml_with_prefix<W: std::io::Write>(
        ...
        if let (Some(raw), Some(original)) = (&self.source_raw, &self.source_original)
            && self.preservation_eq(original)
        {
            writer.get_mut().write_all(raw)?;
            return Ok(());
        }
        let mut buf = itoa::Buffer::new();
        let override_name = qualified(word_prefix, "lvlOverride");
        let ilvl_name = qualified(word_prefix, "ilvl");
        let mut start = BytesStart::new(override_name.as_str());
        start.push_attribute((ilvl_name.as_str(), buf.format(self.ilvl)));
        push_extra_attributes(&mut start, &self.extra_attributes);
        writer.write_event(Event::Start(start))?;
        ...
        if let Some(value) = self.start_override {
            let name = qualified(word_prefix, "startOverride");
            let val_name = qualified(word_prefix, "val");
            let mut element = BytesStart::new(name.as_str());
            element.push_attribute((val_name.as_str(), buf.format(value)));
        ...
        writer.write_event(Event::End(BytesEnd::new(override_name)))?;
        Ok(())
    }

    fn preservation_eq(&self, other: &Self) -> bool {
        self.ilvl == other.ilvl
            && self.start_override == other.start_override
...
impl CT_Num {
    fn from_strict_xml(element: StrictXmlElement) -> Result<Self> {
        let extra_attributes = strict_extra_attributes(&element, &[b"numId"])?;
        let mut descendants = Vec::new();
        let parsed = element.parse(|cursor| {
            ...
            for index in 0..cursor.child_slots() {
                ...
                if child.is_named(Some(W_NS), "abstractNumId") && abstract_num_id.is_none() {
                    ...
                    let mut level_override = CT_LvlOverride::from_strict_xml(child)?;
                    level_override.source_original = Some(Box::new(level_override.clone()));
                    level_override.source_raw = Some(raw);
                    descendants.push(level_override.completeness.clone());
                    level_overrides.push(level_override);
                    position += 1;
                } else {
                    let child = take_element(cursor, index, "numbering instance child")?;
                    descendants.push(unmodeled_element_completeness(child.clone()));
                    extra_xml.push((position, child.into_raw_xml().bytes().to_vec()));
                }
            }
            Ok(Self {
                completeness: StrictXmlCompleteness::default(),
...
impl CT_Numbering {
    ...
    fn preserved_namespace_declarations(&self) -> Result<Vec<(String, String)>> {
        ...
        for num in &self.nums {
            ...
            for level_override in &num.level_overrides {
                ...
                if let Some(raw) = &level_override.source_raw {
                    append_raw_namespace_declarations(raw, &mut declarations)?;
                }
    ...
    pub fn from_xml(xml: &[u8]) -> Result<Self> {
        ...
        let root_attributes = strict_root_attributes(&root)?;
        let mut descendants = Vec::new();
        let parsed = root.parse(|cursor| {
            let mut numbering = Self::new();
            numbering.root_attributes = root_attributes;
            let mut position = 0;
            for index in 0..cursor.child_slots() {
                let Some(StrictXmlNode::Element(child)) = cursor.child(index) else {
                    continue;
                ...
                if child.is_named(Some(W_NS), "abstractNum") {
                    ...
                    let child = take_element(cursor, index, "numbering root child")?;
                    let prefixes = strict_word_prefixes(&child);
                    let at = root_raw_boundary(
                        child.raw_xml().name().qualified.as_bytes(),
                        position,
                        &prefixes,
                    );
...
<w:numbering xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main"
  ...
  <w:abstractNum w:abstractNumId="0"><w:lvl w:ilvl="0">
    ...
    fn property_projection_rejects_excessive_depth_normally() {
        ...
        for _ in 0..=oxml_core::xml::DEFAULT_MAX_XML_DEPTH {
        ...
        assert!(
            matches!(error, crate::OxmlError::InvalidValue(ref message) if message.contains("XML nesting depth")),
            "{error}"
        );
    }
