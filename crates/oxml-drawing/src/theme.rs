...
/// The Office default theme, as Word and PowerPoint ship it.
///
/// Public so the theme adapter's regression can compare a projected `Theme`
/// against one parsed from the same source. The adapter lives in `rdocx-oxml`,
/// which cannot reach a private const here, and a dev-dependency back on
/// `rdocx-oxml` would rebuild the cross-family edge this crate just shed.
