...
`cargo-release` preparation is configured in Cargo metadata. The nine packages
that inherit `[workspace.package].version`, including the unpublished
`rdocx-wasm` and `oxml-py-support` packages, use cargo-release's effective
`workspace` shared-version group and the `v{{version}}` tag template. The exact
published stable family remains the seven packages listed above.
