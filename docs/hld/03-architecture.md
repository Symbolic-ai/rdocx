...
The 14 implemented shared and PowerPoint publication candidates are prepared
at the explicit common incubating version 0.1.3 in their manifests and
workspace pins. The released `rdocx-*` crates continue to use the separate
workspace version. Version preparation and manifest eligibility do not
authorize publication. Registry publication for this family remains pending
until `/release rpptx-v0.1.3` reaches its exact reviewed SHA and receives the
...
14-package family is prepared at 0.1.3 but is not yet published. No existing
